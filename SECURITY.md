...
This document describes the security properties of the MongrelDB Perl client
and how to report vulnerabilities.

## Overview

The MongrelDB Perl client is a Perl module that talks to `mongreldb-server`
...
- The client communicates with `mongreldb-server` over plain HTTP. The
  daemon binds to `127.0.0.1` by default, so traffic stays on the loopback
...
- The native Condition API and query builder accept typed parameters
  (column IDs, typed values), so there is no string interpolation and no SQL
  injection surface. User-supplied values are serialized as typed JSON, not
...
- Binds to `127.0.0.1` only, so it is not accessible from other machines.
- **No authentication by default**, so any local process can query, write,
  or delete data. Enable `--auth-token` or `--auth-users` for any shared
  host.
- No TLS, so traffic is plaintext on the loopback interface.
...
- Server and network errors are mapped to the typed `MongrelDB::Error`
  categories (`auth`, `not_found`, `constraint`, `connection`, `query`),
  not leaked as generic exceptions.
- NaN and Infinity are rejected with a clear `query` error rather than
  corrupting data.

## Dependency security

The MongrelDB Perl client has no runtime dependencies beyond Perl's standard
library (`HTTP::Tiny`, `JSON::PP`). Report dependency vulnerabilities
through GitHub's Dependabot alerts or the private vulnerability reporting
flow below.
...
This keeps the report confidential between you and the maintainers until a
fix is ready. Please include as much as you can:

- a description of the issue and its impact,
- step-by-step reproduction steps,
- the MongrelDB Perl client version, Perl version, and OS,
...
- Progress updates through the private advisory thread until the issue is
  resolved.
- Credit for your responsible disclosure in the advisory, unless you prefer
  to remain anonymous.
